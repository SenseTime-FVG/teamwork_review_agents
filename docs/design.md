...
已保存且启用 Preflight 的仓库在详情页提供“手动执行 CI”。手动执行先更新基础仓库，在默认远端分支最新提交上创建隔离 worktree，然后运行当前保存的全部 CI 步骤。它不伪造 MR/PR 事件、不触发 Agent、不发布 Commit Status，运行来源明确记为手动仓库 CI。手动执行不采用自动门禁的总超时和步骤超时，但必须持久化取消请求并终止当前命令及其完整进程树；自动事件 Preflight 继续遵守原超时。服务关闭或异常退出时，所有未完成手动 CI 收敛为取消或异常终态。
...
仓库 Preflight 增加“发布 CI 结果评论”开关，默认关闭。开关只作用于具有真实 MR / PR 编号的自动事件 Preflight；手动仓库 CI 没有关联编号，不创建、更新或删除平台评论。成功结果也不创建评论，只有命令失败、超时或基础设施异常时才发布失败说明。
...
## 80. Preflight 缓存版本控制边界

仓库级 Preflight 依赖缓存位于 `/data/preflight-cache/`，属于可以由后续 CI 重新生成的本地运行数据，不进入版本控制。项目在根目录 `.gitignore` 中精确忽略该缓存目录，避免持续增长的依赖缓存污染工作区状态或被宽泛暂存操作误收入提交。

忽略范围不扩展到整个 `data/`。数据库、日志、PID、锁、worktree 等内容继续沿用既有细粒度规则，其他新增运行文件仍应在 Git 状态中可见，以便及时发现没有明确归属的数据。增加忽略规则不删除、移动或清理已有缓存，也不影响运行中服务和后续缓存复用。
