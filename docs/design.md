...
“触发规则”同时管理 MR / PR 事件规则和定时触发规则。定时规则包含名称、启用状态、Agent 列表、仓库列表、调度类型、调度参数和时区；调度类型支持固定间隔与标准五段 Cron。固定间隔以正整数分钟、小时或天表示，首次启用或调度定义变化后从当前时间开始计算下一次执行；Cron 按配置时区计算下一次计划时间，时区默认 `Asia/Shanghai`。规则必须至少选择一个 Agent 和一个仓库，不能用“全部仓库”隐式扩大执行范围。
...
管理界面在“触发规则”页提供“MR / PR 事件规则”和“定时触发规则”两个视图。定时规则列表展示状态、调度摘要、时区、Agent 数量、仓库数量和下一次计划时间；详情页独立编辑单条规则，并对 Cron 表达式、固定间隔和时区做前后端一致校验。删除 Agent 或仓库时仍需处理定时规则引用：默认阻止直接删除并在界面显示引用来源，避免静默缩小定时任务范围。
