...
拥有运行工作区的 Agent 结束后执行安全清理：运行成功且工作区无修改、没有新增提交，或者新增提交已经存在于远端引用时立即删除；linked worktree 使用 `git worktree remove`，独立 clone 删除经归属校验的运行目录。失败、超时、存在未提交文件或新增提交尚未推送时保留工作区，并在运行记录中标记“待清理”。保留超过 `runtime.worktree_retention_days`（默认 7 天）后，由独立 `runtime.workspace_cleanup` 计划清理，默认每天 06:00；这项期限包括对遗留本地修改的恢复窗口。定点计划使用主机本地时间，启动不立即清理、停机错过的周期不补跑。清理取得与运行共享的工作区使用租约后重新核验归属、活动任务和期限，结果写回运行状态与日志；旧借用对象库 clone 仅删除自身。计划与结果放在配置历史上方，详见[定时清理设计](design-workspace-cleanup.md)。默认数据库位于项目 `data/` 时，仓库根目录的 `.gitignore` 必须忽略整个 `/data/worktrees/` 运行目录，包括保留标记，避免临时工作区进入版本控制；忽略规则不替代后台安全清理。
...
所有过滤和阶段 125 增加的可选排序都在 SQLite 查询阶段完成，不能先截取固定数量再由浏览器处理。MR / PR 默认按平台快照中的远端 `updated_at` 倒序，事件默认按统一事件载荷中的实际 `occurred_at` 倒序；两类查询都增加稳定次级排序，避免主排序值相同或自动刷新时行顺序跳动。最近扫描时间继续保留用于审计，并可作为 MR / PR 列表的显式排序字段；事件入库时间只用于内部稳定次序。
...
## 33. 历史 PR 最新平台事件参考与手动触发

GitHub PR 的“最新平台事件”来自 Issues 命名空间下的 Timeline Events 接口，不根据当前 `opened`、`closed`、`merged` 快照状态推导。系统按 Timeline 原始顺序选择最后一条能够转换成现有 `change_request.*` 规则事件的活动：关闭、重新打开、合并、提交或强制推送、Draft 切换和标签变化；评论、订阅等尚无统一规则语义的原始活动不作为可手动触发事件。GitLab 当前没有统一活动流实现，不能用 MR 状态伪造最新平台事件。

“最新”只表示该活动在 Provider Timeline 中的位置，不按活动携带的时间字段重新排序。GitHub 的 `committed` Timeline 项可能没有独立的事件创建时间，只能提供提交作者或提交者时间，因此页面展示的“平台提供时间”可能早于 PR 创建时间；该时间只用于审计展示，不能改变 Timeline 顺序或被解释为 PR 创建前已经存在语义事件。
...
运行概览在每个快照行展示缓存的“最新平台事件”。它是可手动重放的 Provider 活动参考，不等同于已经写入事件收件箱的关联事件，因此可能不会出现在 MR / PR 详情的“关联事件”列表中。管理员点击“手动触发”后，系统以当前快照为上下文、以缓存的 Provider 活动为审计来源，生成一个新的 `origin=manual` 语义事件；每次操作使用独立事件 ID，因此可以明确重跑规则。手动事件进入与扫描事件相同的仓库、条件、去重和 Agent 调度流程，没有匹配规则时正常落为 `unmatched`，但不会修改托管平台，也不会额外生成通用 `change_request.updated`。
...
运行概览中的“补发首次事件”和“手动触发最新平台事件”不使用浏览器原生 `window.confirm`。两种操作统一使用应用内确认弹窗，以延续管理界面的深色视觉体系，并让管理员在确认前结构化核对仓库、MR / PR 编号、事件类型、平台提供时间和规则影响。

弹窗由遮罩、标题区、目标信息卡、规则影响提示、安全说明和操作区组成。手动触发必须明确显示缓存的语义事件及平台提供时间；补发首次事件必须明确显示 `change_request.discovered`。当存在事件和仓库范围可能匹配的启用规则时，提示仍需满足规则条件且 Agent 可能执行其获准操作；没有候选规则时说明事件预计记录为未触发。两种操作都要声明不会直接修改远端 PR。
...
2026-09-15 更新：下文的 Git 超时配置键保持兼容，但统一表示“连续无有效进展超时”，不再限制总耗时。标准阶段、对象/文件数量或字节数推进会刷新计时，重复输出和心跳不会刷新。实时采集、跨平台管道读取与安全边界详见 [Git 无进展超时与实时进度](design-git-idle-progress.md)。
...
实时事件另含安全解析后的阶段进度、对象/文件数、字节数、速度、最近有效进展时间、无进展时长及超时类型。Agent Git 详情从持久日志按命令合并最新状态，不再受前 2000 条日志上限影响；旧记录不含新增字段时仍保留展示，并明确其旧超时语义。
...
模型选择目录优先使用已保存 CLI/Home 的 App Server `model/list`，与 `config/read` 共用一次连接，但分别报告失败。模型分页必须完整读取且有界，成功空目录不能回退旧缓存。仅查询失败时采用账号缓存或内置参考目录，并公开标记回退来源与查询时间。界面初始加载、保存配置、进入 Provider 或打开 Codex 详情时更新；手动刷新按钮在只读状态也可用。刷新与表单草稿分离，不修改已选模型，旧配置的迟到请求不能覆盖新目录。具体边界见 [Codex 模型目录刷新设计](design-codex-model-catalog-refresh.md)。
...
运行概览的 MR / PR 列表支持显式进入批量选择模式。默认只显示“选择”按钮；进入后，标题区显示“取消”和带已选数量的“手动触发”按钮，每一行左侧显示复选框。没有缓存最新平台事件的行保留复选框位置但不可选择，并说明需要先完成能够取得最新平台事件的扫描。仓库、状态或展示数量筛选发生变化时退出选择模式并清空选择，避免对已经隐藏的记录执行操作。

批量确认弹窗汇总选中数量、仓库分布和事件类型分布。每个 MR / PR 必须使用自己的缓存最新平台事件创建独立的 `origin=manual` 语义事件，不允许把第一条记录的事件类型套用到其他记录。批量管理 API 对请求目标去重并逐项校验快照、启用仓库和最新 Provider 活动；单项失败不回滚或阻塞其他目标，响应中返回每项的创建状态、事件类型和失败原因。至少成功创建一条事件时只需唤醒一次调度器。
...
目标分支变化事件属于轻量可靠事件。为避免“快照已推进但规则尚未触发”造成永久漏触发，它仍先写入 SQLite 可靠队列，但事件载荷排除 Provider 原始响应，只保存规则匹配、重试和 Agent 上下文需要的数据。服务异常退出时沿用现有事件恢复与幂等调度；处理失败时保留到既有重试流程结束。事件进入完成、未匹配、失败或取消终态后继续留在事件历史中，供运行概览解释目标分支变化及其处理结果。

事件终态收敛时，服务把仓库、PR / MR 编号、标题和地址固化到已经创建的 Agent 运行记录，确保“运行与日志”不依赖事件生命周期仍可展示上下文。终态目标分支事件与实时日志共用 `web.log_retention_days` 保留期，过期清理只删除事件及其调度关系；Agent 运行、最终结果和固化上下文继续遵循各自的持久化与保留策略，不能因事件过期而删除或隐藏已经创建的运行。
...
仓库 Preflight 增加“发布 CI 结果评论”开关，默认关闭。管理界面的开关标签固定显示“失败时发布 PR 评论”，启用或停用状态只由开关控件表达，不能通过切换成否定标签重复表达状态。开关只作用于具有真实 MR / PR 编号的自动事件 Preflight；手动仓库 CI 没有关联编号，不创建、更新或删除平台评论。成功结果也不创建评论，只有命令失败或超时这类已经形成代码门禁结论的结果才发布失败说明；Git、网络、权限、进程启动和工作区清理等基础设施异常只在 Teamwork 内记录并重试，不创建、更新或删除平台评论。

同一仓库、同一 MR / PR 在同一 Preflight 状态上下文下只维护一条活动失败评论。第一次真实失败创建评论；后续真实执行再次失败时先删除原评论，再在时间线底部创建本轮的新评论，避免很早发布且正文很长的历史评论持续停留在时间线上方。复用相同 Head 与配置的失败或超时结果不删除、不更新也不重建现有评论；只有活动映射缺失时才补建，避免同一次 CI 被多个语义事件复用时产生重复通知。评论正文包含稳定隐藏标记、Head SHA、失败步骤、退出码、安全错误摘要和经过脱敏、截断的末尾输出，不发布完整原始日志，也不包含 Provider Token 或其他可识别凭据。

活动评论的远端 ID、状态上下文、Head SHA、源版本代次与内容哈希持久化到 SQLite。刷新和清理按仓库、MR / PR、`preflight` 命名空间与状态上下文加槽位级写锁，并清理该槽位遗留的全部代次映射，确保跨代次并发不会留下多条活动失败评论。后续自动 Preflight 成功时，如果存在活动失败评论，先从平台删除，再清除本地映射；不存在映射时不执行额外平台请求。远端评论已被人工删除时，删除按幂等成功处理；缓存复用失败发现活动映射不存在时允许补建评论，但不得旋转仍存在的评论。
...
真正匹配直接 Agent 的事件继续在调度关系写入后进入 `triggered`；真正匹配 Preflight 规则的事件在本地 CI 结束前继续保持 `processing`，并由界面显示“本地 CI 中”。即时收敛的未匹配事件需要同步固化关联运行上下文并计入已处理数量，后续批次终态循环不得重复完成或重复计数。规则条件、每轮去重、Agent 幂等键、失败重试、服务停止恢复和同一 MR / PR 的批次顺序保持不变。
...
Preflight 失败评论复用同一服务，但使用 `preflight` 命名空间和 `status_context` 槽位，并采用与 Agent 评论不同的“单条活动评论”策略。每次真实执行得到命令失败或超时结论时，删除该槽位所有历史代次评论，再在时间线底部创建本轮评论；相同 CI 结果被后续事件复用时保留现有评论，不执行远端更新或位置刷新，映射缺失时才补建。基础设施异常不代表代码门禁失败，因此不得创建、更新或删除评论，也不得覆盖此前真实的失败结论。当前最新源版本的真实或复用成功结果都会删除该槽位全部活动失败评论和本地映射，避免成功后残留旧代次失败提示；历史旧代次成功不触碰新 Head 评论。数据库升级时保留旧失败评论映射并迁入通用表，首次刷新或成功清理时统一收敛。

自动 Preflight 只把可验证的代码门禁结论写回平台：`success` 发布成功状态，`failure` 和 `timed_out` 发布失败状态。`error` 继续作为可重试的 Teamwork 基础设施异常保存在事件、Preflight 运行和日志中，但不发布平台终态 Commit Status，也不维护失败评论；精确工作区准备失败发生在远端 `pending` 发布之前，因此不会把 Git、网络或权限故障伪装成 PR 代码失败。平台状态写回自身失败时仍保留已得到的本地真实终态，后续事件重试只补发该终态。
...
托管进程首次导入依赖、读取配置并进入 FastAPI lifespan 的耗时可能超过原有 POSIX 5 秒预算。后台 `start` 和 `restart` 在所有平台（含 WSL）统一使用 30 秒默认启动等待；`--startup-timeout` 可用有限正数覆盖本次等待时限，确认就绪后立即返回。健康检查使用无代理连接，不受环境代理影响；失败记录最后未满足的 PID 文件、进程身份或健康响应条件。超时判定继续同时校验 PID 身份、存活状态和健康接口 PID，错误端口所有者不能被识别为成功。详见 [后台启动确认设计](design-background-startup-confirmation.md)。
...
`change_request.target_commits_changed` 在完成、未匹配、失败或取消后不再立即删除。运行概览必须像展示普通语义事件一样展示这些终态记录，使用户能确认目标分支真实 Head 变化发生过、是否命中规则以及关联 Agent 的处理结果。事件继续使用去除 Provider 原始响应的轻量载荷，避免为可观测性复制完整平台数据。

目标分支事件终态收敛时先把仓库、PR / MR 编号、标题和地址固化到关联 Agent 运行，再保留事件本身。后台复用 `web.log_retention_days` 计算统一截止时间，只清理早于截止时间且状态属于 `completed`、`unmatched`、`cancelled`，或状态为 `failed` 且已耗尽配置重试次数的目标分支事件；`pending`、`processing`、`triggered` 和仍可重试的失败事件不允许因保留期清理而丢失。删除事件时可级联删除事件调度关系，但不能删除 Agent 运行、最终结果或已固化的运行上下文。

该保留策略不改变目标分支扫描、首次基线、批次幂等、失败重试或事件处理顺序。默认 30 天保留期以当前事件体量提供足够排障窗口，部署方可通过既有配置统一调整实时日志和终态目标事件的保留时长，无需新增独立清理开关。

## 95. 管理界面下拉弹层统一对齐

管理界面不再使用浏览器原生 `select` 展示单选枚举。macOS 上的 Chrome 会为原生菜单额外预留选中标记和系统边距，使展开弹层相对未展开控件左移或宽度不一致，而且页面 CSS 无法可靠修正。运行概览、运行与日志、仓库、全局环境、Agent、Prompt 文件等页面统一复用页面内单选列表框；弹层以未展开控件为定位基准，左右边界和控件严格一致，并在下方空间不足时向上展开。
...
`codex-cli` 是系统唯一固定内置的模型 Provider：旧配置加载时自动补齐，首次使用时作为全局默认，不允许删除或改变 Provider 身份，但允许管理员停用。它内部提供“基座模式”和“完整 CLI 模式”两种执行方式，默认使用基座模式；前者复用当前 Codex OAuth 登录态并由 Teamwork 执行模型工具循环，后者启动完整 Codex CLI 生命周期。两种模式共享同一个 Provider、账号、模型目录和 Agent 引用，不得把 `codex-oauth` 展示或保存为第二个 Provider。Codex Binary、Codex Home、期望版本、账户登录、账号模型目录、用户 MCP 继承和高级 Codex 配置都归入该 Provider 的详情。外部 API Provider 支持 OpenAI Responses、OpenAI Chat Completions、Anthropic Messages 与 Gemini GenerateContent 协议，保存规范化 Base URL、API Key 凭据引用、请求超时、并发上限、默认参数以及自动发现或手工维护的模型目录。
...
## 98. MR / PR 关联事件多选筛选与排序说明

MR / PR 详情中的关联事件继续由状态存储按事件发生时间、入库时间和事件 ID 依次倒序返回，标题旁明确显示“按时间倒序”，避免用户只能根据列表内容猜测排序规则。排序说明仅描述既有稳定顺序，不允许前端筛选改变事件的相对次序。

关联事件标题区增加状态与事件类型两个多选筛选器。状态选项从当前事件最终展示的状态徽标动态生成，因此“本地 CI 中”“本地 CI 未通过”“Head 已更新，已跳过”等派生状态可以与用户看到的文字一致；事件选项从当前 MR / PR 实际包含的语义事件类型生成。每组内部按“或”匹配，两组之间按“且”匹配，筛选只作用于已经按需加载的详情数据，不增加后台请求或改变全局概览筛选。

每组以空数组表示“全部”。选择“全部”会清除其他具体项，选择具体项会退出“全部”，取消最后一个具体项时自动恢复“全部”。下拉触发器对多项选择显示摘要，结果计数在筛选生效时显示“当前结果 / 总数”，无结果时展示筛选专属空状态。切换或关闭 MR / PR 详情时清空两组筛选；窄屏下筛选控件换行，不能挤压标题、计数或事件状态。

## 99. 最新平台事件命名与 Timeline 顺序

运行概览中的 Provider Timeline 参考统一命名为“最新平台事件”，不能继续简称为“最新事件”。它用于说明当前缓存中可手动重放的最后一条平台活动，与事件收件箱中的扫描事件、手动事件及 MR / PR 详情中的“关联事件”属于不同数据集合。关联事件只展示已经真实入队并具有处理状态的语义事件；最新平台事件在手动触发前可以不出现在关联事件中。

GitHub 最新平台事件继续按 Timeline API 的原始顺序选择最后一条可转换活动，不按 `occurred_at`、提交作者时间或提交者时间排序。列表、空状态、批量选择提示、按钮说明和确认弹窗必须统一使用“最新平台事件”或“平台提供时间”，并明确它是手动触发参考。后端 `latest_event` 兼容字段、Timeline 游标、手动事件生成、规则调度和 GitLab 不支持状态保持不变。

## 100. 历史事件重放与详情页手动触发

事件收件箱中的历史语义事件支持以单条或批量方式手动重放。重放不能修改来源事件的状态、尝试次数、调度记录或 CI 关联，而是复制来源事件保存的事件类型、旧快照、新快照、当前快照、变化字段、Provider 活动审计引用和源版本代次，生成具有独立事件 ID、独立批次 ID、当前触发时间与 `origin=manual` 的新事件。新事件额外记录直接来源事件 ID 和来源事件发生时间，使连续重放仍能逐级追溯；当前仓库不存在或已停用时拒绝创建。新事件进入现有规则匹配、Preflight / CI 与 Agent 调度流程。相同 Head 和配置版本的成功、命令失败、超时或仍有剩余尝试次数的基础设施异常继续按既有机制复用；如果当前基础设施异常已经耗尽自动重试次数，新的手动事件必须保留旧运行审计并创建一条全新 Preflight 运行，重新获得完整重试额度，避免“手动触发”只回显无法再次执行的历史异常。

后端提供按事件 ID 的单条重放接口和最多一百项的批量重放接口。批量请求先去重再逐项校验，单项失败不回滚其他成功项，至少创建一条事件时只唤醒一次调度器。响应必须返回来源事件 ID、新事件 ID、事件类型、创建状态和失败原因。来源事件不存在返回未找到，重复事件、仓库停用或写入失败返回明确冲突；任何路径都不能复用来源事件 ID。

运行概览“最近变化事件”增加与 MR / PR 列表一致的选择模式、单条手动触发入口和批量确认。选择范围只包含当前已加载页面，筛选或翻页时退出选择模式并清空选择。MR / PR 详情中的关联事件保持左键打开详情，右键或键盘上下文菜单打开仅包含“手动触发”的应用内菜单；菜单在点击外部、滚动、关闭抽屉或按 `Escape` 时关闭。所有入口复用统一确认弹窗，明确新事件独立创建、来源事件保持不变，以及后续可能触发 Agent。

MR / PR 详情的当前快照增加独立“最新平台事件”参考，展示平台事件类型和平台提供时间，继续说明它不属于关联事件。详情页顶部在状态与关闭按钮之间增加“手动触发”按钮，复用既有最新平台事件接口与确认语义；没有可用平台事件或 Provider 不支持时禁用并展示原因。详情触发成功后刷新概览和当前详情，但不关闭抽屉。

## 101. 后续批次无匹配事件即时收敛

同一 MR / PR 的前序批次正在执行 Agent、Preflight 或等待失败重试时，调度器仍需持续观察该资源后来入队的事件。后续事件在进入“等待同一 PR / MR 前序批次”或重试退避之前，必须使用当前已热加载的规则配置逐条完成一次只读匹配判定；确定没有任何启用规则匹配的事件立即进入 `unmatched`，不能仅因同一资源存在活动任务而等待前序批次结束。

提前收敛只适用于仍可领取且没有匹配规则的后续事件。至少匹配一条规则的事件继续保留 `change_request_order` 排队原因，等前序批次完整结束后再按原有批次顺序领取、执行 Preflight 和启动 Agent；提前判定不能创建调度关系、运行记录或 Preflight，也不能改变匹配事件之间的顺序、每轮去重和幂等键。仓库不存在或已停用时仍由既有资源处理路径统一收敛，避免在活动循环中引入不同的仓库状态语义。

规则匹配只依赖事件保存的上下文和当前配置，因此无需占用 PR / MR 的串行执行权。为避免活动循环与资源任务竞争同一事件，状态更新必须带有待处理状态约束：只有仍处于 `pending` 的事件才能被提前完成；已经被领取、进入重试或被其他服务处理的事件不得覆盖。规则条件求值异常时不能把事件误判为未匹配，也不能中断调度循环，而应保留事件交给既有批次路径记录失败。即时完成需要清除排队原因、固化终态上下文并准确计入处理数量。服务停止后不再继续提前收敛新事件。

## 102. 事件详情关联 PR / MR 跳转

事件详情页头部的变更请求编号不仅用于说明事件归属，还应作为对应平台 PR / MR 的直接入口。单事件详情数据从事件入库时固化的 `current`、`new` 或 `old` 快照中按优先级解析平台地址，并把地址作为可选摘要字段返回；该字段只增加有界字符串，不返回完整事件载荷或 Provider 原始数据，也不需要数据库迁移。

前端在平台地址存在时把 `#编号` 渲染为新标签页链接，并提供可识别的悬停和键盘焦点状态；仓库名称、事件时间与抽屉关闭行为保持不变。历史事件缺少地址或地址为空时继续显示普通编号文本，不能生成猜测地址或展示不可用链接。链接使用 `noopener` / `noreferrer` 隔离新页面，不触发抽屉的内部层级跳转。

## 103. 关联事件右键菜单视口定位与光标反馈

MR / PR 详情抽屉使用滑入 `transform` 动画，该动画会让抽屉成为固定定位后代的包含块。关联事件右键菜单如果继续渲染在抽屉 DOM 内，并把浏览器视口的 `clientX`、`clientY` 直接作为 `position: fixed` 坐标，就会重复叠加抽屉左侧偏移，导致在事件左侧右击却从页面右侧展开。

右键菜单必须通过 React Portal 直接渲染到 `document.body`，以浏览器视口作为唯一定位坐标系。首次渲染后读取菜单实际宽高，在距离视口边缘至少八像素的范围内校正位置；右击靠近右边或底部时分别向左或向上收回，不能依赖单个菜单项的硬编码宽高。抽屉滚动、窗口缩放、点击外部、`Escape` 和详情关闭继续关闭菜单，菜单内部点击不得被外部关闭监听提前吞掉。

关联事件整行同时支持左键查看和右键操作，必须在整行及其文字、状态和箭头子元素上保持手指光标；菜单项及其子元素同样使用手指光标。禁用状态仍使用默认光标。光标和定位调整只增强交互反馈，不改变历史事件重放、确认弹窗、事件详情层级或键盘上下文菜单语义。

## 104. Preflight 失败评论时间线刷新与成功清理

Preflight 失败评论面向“当前门禁异常”而不是长期审核历史。对于一次新执行得到的 `failure` 或 `timed_out`，服务必须在同一槽位级资源租约内枚举所有历史代次映射，逐条删除远端评论并立即删除对应本地映射，全部删除完成后才创建本轮评论。创建失败不得恢复旧评论，也不得改变真实 CI 终态；删除中途失败时停止创建，保留尚未成功删除的映射供后续重试，避免出现两条活动评论。

缓存复用不是新的 CI 执行，不应为了每个语义事件再次通知或把评论推到时间线底部。复用失败或超时结果时，如果该槽位已有任意活动映射则不做远端请求；如果映射不存在，说明先前发布失败或评论生命周期尚未建立，可以补建当前失败评论。当前最新源版本代次的复用或新执行成功结果都删除该槽位的全部失败评论，而不是只按当前事件携带的源版本代次清理，确保旧代次失败结论不会在最新 Head 通过后继续误导用户；历史手动事件携带的旧代次成功不得删除更新 Head 的活动失败评论。

Agent 托管顶层评论仍按源版本代次保留历史并在同代次内更新，不能受 Preflight 单活动评论策略影响。`error` 与 `superseded` 继续不触碰平台评论。评论删除或创建错误只写入脱敏且有界的 Preflight 日志，Commit Status、Preflight 结果、Agent 是否启动以及事件重试语义保持不变。

## 105. MR / PR 详情标题编号平台跳转

MR / PR 详情抽屉顶部的变更请求编号应与当前快照中的“平台地址”保持一致，直接使用变更请求记录已经保存的 `web_url` 作为对应 GitHub PR 或 GitLab MR 的入口。标题区只把 `#编号` 渲染为链接，仓库名称、标题、分支、状态、手动触发和关闭交互保持不变，不增加接口字段或数据库迁移。

链接使用新标签页打开，并通过 `noopener` / `noreferrer` 隔离外部页面；提供可识别的链接颜色、下划线、键盘焦点状态和手指光标。历史或异常记录缺少平台地址时降级为普通编号文本，不能根据仓库或 Provider 猜测 URL，也不能生成不可用链接。

## 106. Provider 危险操作应用内确认

Provider 详情中的未保存草稿与删除操作必须使用项目统一的应用内确认弹窗，不能调用浏览器原生 `confirm`。放弃草稿弹窗展示 Provider 名称、ID 和当前配置版本，使用“继续编辑”和“放弃修改”两个明确动作；删除弹窗展示全局默认与 Agent 引用影响，继续沿用危险操作视觉层级。遮罩、关闭按钮和 `Escape` 都取消当前意图，不得误执行放弃或删除。

关闭详情、切换 Provider、新建 Provider 和取消编辑遇到脏草稿时，需要先保存待执行动作；用户确认后再清理草稿并完成原操作，取消确认则保持表单、选中 Provider 和输入内容不变。删除确认后继续调用既有管理接口并显示忙碌状态，成功时关闭详情，失败时关闭确认弹窗并保留原详情用于重试。该调整只改变交互展示和动作编排，不修改 Provider 配置、密钥存储、引用回退或热加载语义。

## 107. Provider 详情错误就近展示

Provider 详情中的保存、连接测试、密钥查看或替换、模型目录刷新和删除请求都发生在覆盖页面主体的抽屉内，其错误不能继续只写入抽屉外侧的页面级提示。详情操作异常应固定展示在抽屉标题栏下方、滚动内容上方，保证用户位于任意滚动位置都能立即看到；长错误允许换行和有限高度滚动，并提供明确的关闭按钮与 `role=alert` 语义。

每次开始新的详情操作时清除旧的详情错误和外层遗留错误，成功结果继续沿用现有通知或连接结果展示；切换、关闭或重新打开 Provider 时清除旧详情错误，避免跨 Provider 串扰。Provider 列表及快照在没有打开详情时加载失败，仍使用页面级错误提示；详情打开期间的快照加载异常改为抽屉内展示。错误展示层级调整不改变接口调用、重试、配置保存、密钥保护、删除确认或引用回退语义。

## 108. Provider 列表身份文字可读性

Provider 列表第一列承担配置身份识别，需要让显示名称和 Provider ID 在不打开详情的情况下即可快速辨认。显示名称字号从偏小的表格辅助文字提升到 `14px` 并保持较强字重，Provider ID 提升到 `11px`，两行继续使用主次颜色区分并保留省略号，避免长名称撑开表格。

本次只调整身份列文字和两行间距，不改变头像尺寸、表格列宽、行高、其他字段、状态徽标或点击行为。窄屏卡片布局继续复用同一身份样式，确保桌面与移动视图保持一致的可读性。

## 109. Codex 上游错误原因可见性

Codex 模型基座的 SSE 回合失败不能只显示“Codex 上游报告模型回合失败”。客户端应从 `response.failed`、`error` 事件以及非 2xx HTTP 响应中提取上游明确提供的 `message`、`type`、`code`、`param` 和请求 ID 等有限字段，并将其整理为可读的失败原因。没有可用原因时继续显示现有通用文案，避免把协议不完整误报为具体上游错误。

错误详情只允许进入既有 Agent 运行和事件 `error` 字段以及运行日志的错误摘要，沿用详情页现有展示链路，不新增数据库列或迁移。提取过程不得保存完整上游载荷、Prompt、响应内容、请求头或凭据；字段需清理控制字符、限制长度，并遮蔽 Bearer、API Key 等常见敏感值。HTTP 状态码、错误类型和请求 ID 仅在上游确实返回且经过脱敏时展示。

同一错误仍遵循现有重试语义：已收到 SSE 事件后的模型回合失败不自动重试，尚未收到事件的 502/503/504 与传输异常继续按原策略重试。前端事件详情、Agent 运行结果和消息流直接使用脱敏后的错误文本，使用户能看到真实原因而不会暴露上游原始数据。补充 SSE、HTTP JSON、嵌套错误结构、空字段、超长文本和敏感值脱敏测试；不改变 CI、评论、规则匹配或服务生命周期行为。

## 110. Agent 基模分级回退策略

模型基座调用失败时，系统按一次运行开始时解析出的有序候选链逐级尝试，不重新创建 Agent 运行、不重置总超时、不重复执行已经完成的工具调用。全局配置维护“默认模型 + 全局回退链”，Agent 可另外维护有序的 Agent 级回退链。若 Agent 显式指定主模型，则候选顺序为“Agent 主模型 → Agent 级回退链 → 全局默认模型 → 全局回退链”；若 Agent 未指定主模型，则以全局默认模型作为主模型，再接 Agent 级回退链和全局回退链。整条链按 `(Provider, Model)` 身份去重，保留第一次出现的位置，因此 Agent 主模型或 Agent 级节点与全局节点重复时不会重复请求。

只有模型 Provider 不可用类错误才允许切换候选，包括认证或额度拒绝、限流、上游暂时性 5xx、连接/超时、模型不存在或 Provider 未配置凭据等。Prompt 或工具协议错误、参数校验失败、工具/Git/工作区失败、取消、Agent 总超时、输出校验失败不触发回退，直接结束当前运行。Provider 被停用时跳过该节点；所有候选失败后返回失败结论，并保留每个候选的脱敏尝试结果。

模型回合切换时继续使用已经构建的规范化对话历史，包含已完成的工具调用和工具结果，不回放副作用操作。候选链在运行开始时固化，但每次回退仍复用本次运行的取消信号和剩余超时预算。Codex CLI 的基座模式与外部 API Provider 统一纳入该策略；不透明的完整 Codex CLI 子进程不在运行中自动切换，避免重放不可逆副作用。

运行快照与详情必须同时展示请求的主模型、Agent 级回退、全局回退、每次尝试的 Provider/Model、失败原因、最终成功节点和是否发生回退。日志与错误文本继续脱敏，不得保存 API Key、Token、完整 Prompt 或上游原始响应。删除 Provider 时清理全局和 Agent 回退链中的引用；若删除的是主模型，沿用现有回退到 Codex CLI/全局默认模型的规则。配置验证拒绝不存在的 Provider 和非法空模型组合；重复节点允许配置但在运行链展开时按 `(Provider, Model)` 去重，旧配置缺少回退字段时按空链兼容。

## 111. 回退模型编辑器控件对齐

全局模型回退链和 Agent 级模型回退链使用统一的紧凑编辑行。每行按“备用序号、Provider、模型、操作”稳定分列，Provider 与模型都放在带字段标签的表单容器中，标签、输入框字体、行高、内边距和控件高度保持一致；Provider 下拉不得继续继承页面根字号，避免比模型输入框明显偏大。模型帮助说明允许占用自身字段空间，但不能改变 Provider 与模型输入框的顶部基线。

回退行中的排序和删除按钮固定在操作列并垂直居中，序号与字段内容不因 Provider 名称或模型帮助文本变化而错位。窄屏布局继续将字段安全换行，保持 Provider、模型和操作入口可用，不改变现有回退链顺序、Provider/Model 数据、去重和运行时解析语义。本次只调整编辑器组件结构与样式，不增加配置字段、不迁移数据库，也不影响 Provider 详情页和全局默认模型控件。

## 112. Provider 草稿模型目录检测

外部 API Provider 新建或编辑时不要求用户先填写占位模型。用户填写 Base URL 和 API Key 后，详情表单提供显式“检测模型”按钮；按钮只在用户主动点击时发起请求，不在输入过程中自动访问上游。检测请求使用当前未保存草稿的驱动协议、Base URL 和 API Key，新建 Provider 不要求先保存，已保存 Provider 在没有新 Key 时复用受管凭据。检测过程不写入配置、凭据、日志或运行历史，只返回规范化模型 ID 及有界错误信息。

检测成功后，详情表单展示可选模型列表，用户选择其中一个作为 Provider 默认模型；检测到的完整列表同步进入当前草稿的模型目录，保存时与默认模型一并提交。用户也可以跳过检测，直接在手工模型目录中逐行维护模型 ID；手工输入和检测结果都遵循现有去重、保序和默认模型校验。检测失败不保存半成品 Provider，错误应说明 HTTP 状态、接口不支持、认证失败、连接失败或响应格式异常等真实原因，同时保留手工填写路径。

后端新增只读草稿检测接口，校验驱动协议和 Base URL 后调用既有模型目录发现逻辑；API Key 只作为本次请求参数使用，任何异常均执行现有脱敏与长度限制。已保存 Provider 的刷新模型接口继续保留，改为复用同一发现逻辑。配置模型不新增模型来源字段，旧配置和现有模型目录按原语义兼容；最终保存仍要求外部 Provider 至少配置默认模型或非空模型目录，避免检测失败后产生不可运行配置。

前端检测结果应在 Provider 表单内就近展示，明确检测状态、候选数量和当前选择；模型候选使用可键盘操作的单选列表，检测中的按钮禁用并显示进度，错误可关闭且不覆盖页面级状态。切换 Provider、关闭详情或开始新的检测时清理旧结果，保存成功后沿用现有热加载和通知语义。检测按钮、候选列表和手工文本框在窄屏下安全换行，不改变 API Key 脱敏、连接测试、删除确认及既有 Provider 运行行为。

## 113. GPT 模型推理 effort 能力边界

推理 effort 属于具体模型能力，不是所有模型或 Provider 的通用参数。OpenAI Responses、Chat Completions 等外部 API Provider 只有在当前模型属于 GPT 系列时才允许设置和发送 effort；模型 ID 以大小写不敏感的 `gpt-` 前缀识别。Codex CLI 基座继续沿用其模型目录声明的推理等级。Anthropic、Gemini、DeepSeek 及其他非 GPT 模型不支持该参数，运行时必须完全省略对应请求字段。

全局默认模型、全局回退链和 Agent 回退链的模型节点可保存可选 `reasoning_effort`。节点明确值优先于 Agent 级推理设置，Agent 未明确设置时再使用 Provider 默认；如果最终模型不支持 effort，所有继承或历史值都被忽略且不发送。Provider 级默认 effort 仍保留作为 GPT 模型的兜底，但不能使非 GPT 模型获得推理参数。

管理界面只在解析出的模型支持 effort 时展示可选项，并提供“跟随 Provider 默认/不显式传递”入口；模型切换到非 GPT 后不再展示有效 effort。模型目录或模型 ID 暂不能确认能力时按不支持处理，避免向上游猜测发送未知参数。运行快照和回退尝试记录实际采用的 effort 及来源；非 GPT 记录为空或“不支持”，不伪造默认值。该调整兼容旧配置，不需要数据库迁移。

## 114. 外部 Provider 连接测试兼容性与上游错误可见性

外部 Provider 的连接测试只验证当前保存凭据、Base URL 和默认模型能否完成最小请求，不应为了测试而强制发送固定的 `reasoning.effort`。连接测试省略推理 effort 和工具声明，让上游模型使用自身默认值，避免把某个模型不支持的等级（例如 `minimal`）误报成连接失败；正式 Agent 运行仍按阶段 113 的模型能力和配置规则传递推理参数。

外部 Provider 返回非 2xx 时，客户端从 JSON 错误正文及受限请求头中提取 `message`、`type`、`code`、`param` 和请求 ID 等白名单字段，组合为可读的 HTTP 错误原因。正文无法解析或没有可用字段时保留稳定的 HTTP 状态通用文案。错误文本必须清理控制字符、限制长度并遮蔽 Bearer、API Key、Token 及 URL 查询凭据，不得保存完整响应正文、请求头或请求载荷。该错误沿用现有 `ModelProviderRequestError` 和详情页错误展示链路，保留状态码与模型回退判定。

模型目录检测和连接测试使用同一套有界错误提取规则；检测失败仍不写入 Provider 草稿或凭据。前端不新增错误展示层，继续在 Provider 详情抽屉内显示后端返回的具体原因。补充连接测试不发送固定 effort、GPT 模型 HTTP 400 真实原因、嵌套错误字段、请求 ID、非 JSON 正文、超长文本和敏感值脱敏测试；不改变正式运行的推理参数、回退、CI、评论或服务生命周期语义。

## 115. README 近期能力与使用流程同步

README 作为首次使用和运行排障入口，应与已经落地的 Provider、模型回退、事件手动触发和本地 CI 行为保持一致。文档只描述当前代码和配置已支持的流程，不把设计中的未来能力写成现状，也不复制过多实现细节。

Provider 快速开始应说明新建或编辑时可以先填写 Base URL 和 API Key，再点击“检测模型”读取目录并选择默认模型；检测失败仍可改用手工模型目录。Provider 章节需要同步四类外部协议、凭据隔离、连接测试不强制发送固定推理等级、HTTP 错误在详情抽屉中展示真实且脱敏原因，以及 GPT 模型 effort 与非 GPT 模型参数过滤规则。

模型回退说明应明确 Agent 主模型、Agent 级回退、全局默认和全局回退的顺序，说明仅 Provider 不可用类故障触发回退、候选按 `(Provider, Model)` 去重且运行历史固化实际尝试。运行概览章节继续保留最新平台事件、列表/详情手动触发和批量触发语义，不重复引入另一套“最新事件”定义。

README 仅增加面向用户的流程、配置提示和排障入口，具体实现契约仍以 `docs/design.md`、`docs/implementation-plan.md` 及专项文档为准；本轮不修改 API、配置模型、数据库或运行行为。

## 116. CI 与 Agent 详情编号平台跳转
...
## 118. 新建 Agent 的默认执行权限

“添加 Agent”生成的前端草稿默认采用工作区可写沙箱、允许命令联网，并勾选 `MR / PR 写操作` 与 `本地仓库写操作` 两项声明，使新建 Agent 开箱即具备修改工作区、提交或推送以及执行平台写操作所需的显式边界。命令联网不预设域名白名单，仍由运行时沙箱按现有规则控制；HOME 模式继续默认继承系统 HOME，其他模型、超时、Skill、sub-agent 和评论选项保持原有初始值。

该默认只作用于管理界面新建 Agent 时生成的草稿，不改变已保存 Agent、直接编辑的 YAML 或后端 `AgentConfig` 缺省值。用户仍可在保存前取消任一写操作声明、关闭命令联网或改回只读沙箱；表单现有联动校验继续保证沙箱与写操作声明一致，避免历史配置被隐式提权。

## 119. 环境变量来源下拉可读性

Agent 和全局环境变量编辑器中的“变量来源”下拉需要为“固定值”和“宿主机环境”保留完整可读空间，不能因环境变量行采用紧凑多列布局而把选项文字截断。来源列在桌面、中等屏幕和窄屏分别设置足够的最小宽度，下拉菜单沿用触发器宽度和既有省略策略；变量名、变量值、Secret / Prompt / 进程开关及删除按钮的相对顺序保持不变。

本次只调整环境变量行的局部网格列宽和响应式断点，不改变变量来源含义、优先级、脱敏、Prompt / 进程暴露或保存数据结构。窄屏仍允许变量值列收缩或换行，来源选项不能被遮挡，也不能造成页面主体的横向溢出。

## 120. 扫描周期按 MR / PR、源分支和目标分支去重

触发规则提供三个相互独立、可叠加的扫描周期去重开关：`deduplicate_per_scan`（按 MR / PR）、`deduplicate_source_branch_per_scan`（按源分支）和 `deduplicate_target_branch_per_scan`（按目标分支）。三个开关均位于“触发规则”详情编辑页，与执行仓库 CI、工作区继承等规则选项并列；旧配置缺少新增字段时按 `false` 兼容读取。

规则先按事件类型、仓库范围和条件完成候选匹配，再在同一扫描批次、同一规则和同一 Agent 的候选事件中执行去重。开启的任一维度相同即视为同一去重组：按 MR / PR 使用 Provider、仓库 ID 和编号，按分支使用 Provider、仓库 ID 和完整分支名，避免不同仓库或不同平台的同名分支相互影响。三个开关全部关闭时保持逐事件触发；只开启一个或多个开关时，去重条件按“或”组合，而不是要求所有维度同时相同。

每个去重组只选择时间最新的候选事件触发一次 Agent 或 Preflight，选择顺序使用事件发生时间并以事件 ID 稳定打破并列。较早候选事件直接进入 `unmatched`（界面显示“未触发”），不写入 Agent / CI 调度关联、不计入运行次数，也不改变事件原始载荷；事件详情可展示“本扫描周期内已被更新事件替代”的去重原因。运行详情的关联事件仅包含真正触发的最新事件，Agent 的 Prompt、环境、工作区、锁和幂等键均来自该代表事件的最终快照。

去重范围严格限定为一次扫描生成的 `batch_id`，不会把新扫描周期、手动重放或历史待重试事件挂接到之前的 Agent 运行。不同规则、不同 Agent 分别计算去重组；同一最新事件可为同一规则下的多个 Agent 各自产生一次运行。去重只抑制旧事件的调度，不删除事件历史，也不改变没有候选规则事件的既有未触发收敛和同一 MR / PR 批次顺序。

验收：规则页能分别保存和展示三个去重开关；同周期同规则同 Agent 的同 MR / PR、同源分支、同目标分支及混合条件均只由最新候选事件触发，旧候选显示未触发且 Agent 列为空；不同周期、不同仓库、不同规则或不同 Agent 不会错误合并，关闭全部开关时既有逐事件行为保持不变。

## 121. 定时触发规则与默认分支独立运行

“触发规则”同时管理 MR / PR 事件规则和定时触发规则。定时规则包含名称、启用状态、Agent 列表、仓库列表、调度类型、调度参数和时区；调度类型支持固定间隔与标准五段 Cron。固定间隔以正整数分钟、小时或天表示，首次启用或调度定义变化后从当前时间开始计算下一次执行；Cron 按配置时区计算下一次计划时间，时区默认 `Asia/Shanghai`。规则必须至少选择一个 Agent；仓库列表为空时动态表示当前全部已启用仓库，明确选择仓库时只执行所选范围。

每个到期周期都必须生成任务，不因上一个周期仍在排队、准备或运行而跳过。一次到期按“仓库 × Agent”拆分为互不共享运行 ID 的根运行；每个根运行使用独立工作区，并继续受全局并发数、Agent 并发数和写资源锁约束。并发额度不足只进入队列，不丢弃本周期；同一仓库同一分支上的写操作仍可被资源锁串行化，但这不改变两个周期已经分别创建、可独立审计和取消的事实。

调度器为每次到期生成稳定的 `occurrence_id`，并在 SQLite 中以规则名、调度定义版本和计划时间建立唯一约束。服务在一个到期点重复唤醒、配置热加载或短暂重入时不得重复创建同一周期；运行中的服务必须把每个已经到期的周期持久化后再异步执行。服务停机期间不补跑历史周期，重新启动时从当前时间计算下一次计划，避免意外制造积压洪峰；服务暂停期间同样不创建定时运行，恢复时不回补暂停窗口。

定时运行不创建 `ChangeEvent`，不写入 MR / PR 事件历史，也不使用虚假的 `#0`。执行器接收独立的定时上下文，包含规则名称、计划时间、实际创建时间、周期 ID，以及仓库 ID、项目、平台类型、远端默认分支和该分支当时解析出的 Head SHA。根 Agent 的 Prompt 以 `schedule` 字段呈现该上下文；环境中提供仓库、运行、定时规则、计划时间、周期 ID、分支和 Head SHA，不提供 `MR_*` 或 `EVENT_TYPE`。sub-agent 继续继承仓库和父任务边界，并沿用父运行的定时来源，保证调用链可以审计但不会获得虚假的 MR 信息。

每个周期执行前更新基础仓库并解析 `origin/HEAD`；无法识别远端默认分支或读取 Head SHA 时，该“仓库 × Agent”运行以失败结束并记录真实错误。工作区从该 SHA 创建独立 clone 或 linked worktree，Agent 的沙箱、Provider、模型、回退链、环境、Skill、sub-agent、超时和取消语义保持不变。定时根运行不执行 MR / PR Preflight，不回写平台状态，也不能调用托管 MR / PR 评论工具；Agent 即使配置了托管评论，定时 Prompt 也不注入发布评论要求，工具调用时返回明确的“当前运行没有 MR / PR 上下文”。

管理界面在“触发规则”页提供“MR / PR 事件规则”和“定时触发规则”两个视图。定时规则列表展示状态、调度摘要、时区、Agent 数量、仓库范围和下一次计划时间；详情页独立编辑单条规则，并对 Cron 表达式、固定间隔和时区做前后端一致校验。删除 Agent 或被定时规则明确选择的仓库时仍需处理引用：默认阻止直接删除并在界面显示引用来源；空仓库列表表示动态全选，不构成对某一个仓库的显式引用。

“运行与日志”把定时根运行标记为“定时触发”，显示定时规则、计划时间、周期 ID、仓库、默认分支和 Head SHA；MR / PR 运行继续显示编号和平台链接，sub-agent 显示继承的根来源。定时运行可按现有状态、Agent 和仓库筛选，详情中的触发来源不得回退成 `resource_key` 或伪造编号。

验收：固定间隔与 Cron 均能按配置时区产生到期周期；上一周期未结束时下一周期仍创建独立根运行，受并发额度排队但不丢失；同一到期点重复唤醒不重复创建；不同仓库和 Agent 分别运行；暂停或停机窗口不补跑；Prompt、环境、工作区和日志均使用真实默认分支与 Head SHA，且不出现虚假 MR / PR、事件记录、评论或 Preflight 行为。

## 122. 仓库级 Provider Token 覆盖

Provider 的 `token_env` 是平台凭据的逻辑变量名。每次对具体仓库调用 GitHub 或 GitLab API 时，Teamwork 按“仓库环境变量 → 全局环境变量 → 服务进程宿主机环境变量”解析该变量，使绑定同一 Provider 配置的不同仓库可以使用不同 Token，同时继续共享平台类型、Base URL 和请求超时。仓库层与全局层都支持固定值和 `from_system` 引用；推荐仓库层把同一个逻辑变量名分别映射到不同的宿主机变量，避免把真实 Token 明文写入配置文件。

优先级按“是否配置变量名”决定，而不是按解析结果是否非空决定。如果仓库层已经声明 Provider Token 变量，但固定值为空或引用的宿主机变量不存在，则当前仓库的平台调用应明确失败，不得回退到权限可能更大的全局或宿主机同名 Token；全局层已经声明但解析为空时同理。只有该层完全没有声明变量名时才继续读取下一层。
...
## 124. 定时规则动态全仓库与 sub-agent 工作区继承

定时规则的 `repositories` 与 MR / PR 规则保持一致的范围语义：空数组或省略字段表示每次到期时动态选择配置中全部已启用仓库，非空数组表示只选择其中仍然启用的仓库。动态解析发生在周期实际分发时，因此后续新建并启用的仓库会自动进入全仓库规则，停用或删除的仓库会自动退出；显式选择到停用仓库时继续记录明确错误。空范围不是对每个仓库的静态引用，删除任一仓库不需要先修改该规则。

定时规则新增 `inherit_workspace`，默认关闭，语义与 MR / PR 规则完全一致。开启后，根 Agent 通过内嵌或 MCP 方式调用 sub-agent 时，sub-agent 复用父 Agent 当前临时 clone 或 linked worktree，可见当前默认分支快照上的分支切换、暂存区和未提交文件；关闭时，sub-agent 仍以同一次定时上下文和父运行固化的 Head SHA 创建独立工作区。该开关只控制文件系统与 Git 状态，不继承父 Agent 对话、模型、Skill 或权限配置。

界面把空仓库选择明确显示为“全部已启用仓库”，保存时不再将零选择视为校验错误；列表中的仓库范围也显示“全部”而不是“0 个”。定时规则详情在仓库选择下方提供与 MR / PR 规则相同的工作区继承开关和风险说明。

验收：空仓库定时规则可创建和保存，并在每次周期覆盖当时全部已启用仓库；显式仓库范围保持原行为；新启用、停用和删除仓库能动态进入或退出全仓库规则；开启工作区继承后 sub-agent 收到 `inherit_workspace=true` 并复用父目录，关闭时保持独立工作区；旧配置缺少字段时继续按全仓库、独立工作区读取。

## 125. 运行概览列表排序

“已扫描 MR / PR”和“最近变化事件”的排序必须在 SQLite 完成筛选后的完整结果集上执行，再应用分页，不能只重排当前页面。两个列表分别保存自己的排序字段与方向；修改任一排序条件时返回第一页，并清理当前列表的批量选择，仓库范围、状态、编号和展示条数筛选保持不变。

“已扫描 MR / PR”支持按 MR / PR 编号、远端更新时间、最近扫描时间和最新平台事件时间排序，默认按远端更新时间从新到旧。“最近变化事件”支持按 MR / PR 编号和事件时间排序，默认按事件时间从新到旧。排序入口直接放在对应列表的列标题中，不在列表上方增加“排序 / 顺序”下拉框。点击非当前排序列时按降序排列，再次点击当前列切换升序或降序；编号按数值大小、时间按新旧排序。当前排序列高亮并显示方向箭头，其他可排序列显示双向提示。

可排序表头使用原生按钮，支持键盘聚焦与 Enter / 空格操作，所在表头通过 `aria-sort` 表达当前方向；按钮提示说明下一次点击的排序行为。两个列表继续复用各自的筛选回调，点击表头后只重置对应列表的页码和批量选择，不改变另一个列表的排序。

排序参数由管理 API 使用固定枚举校验，SQLite 排序表达式只能从后端白名单映射生成。主排序值相同时，以仓库 ID、MR / PR 编号和稳定记录 ID 作为确定性次序，避免翻页期间记录随机移动。按最新平台事件时间排序时，没有可识别时间的记录无论升序或降序都位于末尾；其他可空排序值沿用同样的空值末尾原则。旧客户端未传排序参数时继续使用两个列表各自的默认顺序。
