...
Preflight 失败评论复用同一服务，但使用 `preflight` 命名空间和 `status_context` 槽位。同一源代次连续命令失败或超时时更新当前失败评论；源代次变化后的首次失败追加新评论。基础设施异常不代表代码门禁失败，因此不得创建、更新或删除评论，也不得覆盖此前真实的失败结论。某一代次随后成功时只删除该代次的失败评论和本地映射；新代次第一次即成功时不创建评论，也不删除旧代次历史失败评论。被人工删除的失败评论同样只在下一次真实失败发布时重建。数据库升级时保留旧失败评论映射并迁入通用表，避免部署后无故重复评论。
