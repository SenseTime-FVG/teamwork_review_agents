...
CI 子进程只隐式继承工具运行所需的 PATH、临时目录、语言、证书、代理和缓存变量，并注入 `CI=true`；`HOME` 指向一次性空目录。Provider Token 默认只由后台确定性接口用于扫描和 Commit Status 回写；管理员明确开启对应变量的“进入进程”后，它也会传入仓库 CI。Codex/OpenAI 模型凭据不会通过该开关进入被测代码。把 Provider Token 暴露给 CI 意味着仓库代码能够读取并外传该凭据，只应在可信代码和最小权限 Token 场景下使用。
