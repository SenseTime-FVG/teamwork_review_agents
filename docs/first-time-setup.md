...
服务启动后，进入“运行概览”或“仓库”，点击“一键配置”。没有仓库时会显示首次使用提示。

1. **选择平台**：GitHub、GitLab，或自定义 GitHub Enterprise / 自建 GitLab 的 API 地址。自定义并不代表支持任意代码托管平台。
2. **仓库与凭证**：填写 HTTPS 或 SSH 地址、Token（可用小眼睛查看），也可引用服务进程的宿主机环境变量。工作目录和 ID 自动生成。已有项目应明确选择“更新”，不会重复创建或迁移原目录。
3. **选择规则**：从当前所有事件规则、定时规则中勾选此仓库需要的规则。以后添加的规则会自动出现，不限定为两个内置规则；名称、触发时机、关联 Agent 和原有范围都会展示。
4. **配置 Skill**：默认不使用，明确禁止本仓库装载 Skill。开启后可选择已注册 Skill，或引用包含 `SKILL.md` 的服务端目录，再按 Agent 分配；包含规则允许调用的 sub-agent。电脑上的文件夹可先在 SKILL 页导入，再重新打开向导选择。本仓库的分配不会修改其他仓库共用的 Agent。
5. **检查与完成**：核对仓库、凭据变量和最终规则范围，可执行只读连接检查。最后点击“完成配置”一次保存所有配置，按所选规则正常调度，不额外立即执行 Agent。模型始终沿用 Agent 配置，未指定时继承全局默认。

规则仓库选择为空表示“全部（包括未来新仓库）”。原来是全部且本次勾选，保持全部；本次不勾选，则改成旧仓库白名单，暂时停用的旧仓库也保留原适用关系。已有白名单只加入或移除当前仓库。原关闭规则本次勾选时只为当前仓库启用；没有任何适用仓库时关闭规则，不能用空名单表示“没有”。

连接检查只验证平台仓库读取与 `git ls-remote`，不验证推送/合并权限，也不启动模型。SSH 地址需要服务账号已有的 SSH 密钥和可信主机记录；不支持通过明文 HTTP 传 Token。固定 Token 会写入本地 `config.yaml`，请保护该文件；API 展示与配置历史脱敏。避免明文落盘可选择宿主机变量引用。新填凭据自动配置 Secret 开、Prompt 关、进程开，不覆盖其他仓库凭据。

向导途中取消不保存。若其他管理员已修改配置，保存会拒绝覆盖并提示重新打开向导。要逐项配置或调整高级选项，继续阅读下方原有配置流程。
