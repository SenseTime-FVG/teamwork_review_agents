...
Agent 运行环境按“仓库 > Agent > 全局”解析。同名变量整项覆盖（值、来源、Secret 和暴露开关），不是逐字段合并；仓库配置为空或引用不到宿主机值时不会自动回退。系统运行变量始终由服务注入。子 Agent 使用自己的 Agent 配置与当前仓库配置，不继承父 Agent 的变量。
...
后台扫描和平台 API 使用的 Provider Token 按仓库环境、全局环境、服务进程宿主机环境的顺序读取同名变量，不读取 Agent 配置。仓库层和全局层推荐使用 `from_system`，避免把真实值写入 `config.yaml`；某层已经配置但解析为空时不会向更宽权限层静默降级。
...
启动和重启成功后会输出访问地址，例如监听 `0.0.0.0` 且配置管理员 Token 时：

```text
监听地址：0.0.0.0:8080
本机访问：http://127.0.0.1:8080
局域网访问候选（以实际网卡名称为准）：http://192.168.1.20:8080
认证方式：需要管理员 Token（本机与局域网相同）
```

上面的网卡和 IP 仅为示例；实际输出取自已启用网卡，不通过外网探测。多网卡分别列出，停用、回环和链路本地地址不作为局域网候选；检测失败时提示检查网卡，不影响启动。监听 `127.0.0.1` / `localhost` 时只显示本机地址，指定具体 IP 时只显示该地址；监听 `::` 时显示 IPv6 地址，不假设已同时开放 IPv4。前台 `run` / `serve` 就绪后同样输出，后台子进程的输出也写入后台日志。

这些地址只是候选，VPN、容器、WSL 网络隔离及防火墙可能影响访问；无需为了打印地址修改网络设置或开放额外端口。配置管理员 Token 后，本机和远程仍执行相同认证。
...
保留期限由 `runtime.worktree_retention_days` 控制，默认 7 天。`runtime.workspace_cleanup` 默认每天 06:00 独立检查，不再等到仓库下次运行。使用后台电脑本地时间；服务启动不立即清理，停机错过的计划不补跑。

在“全局配置与环境”的“配置历史”上方，可通过“工作区清理”卡片配置固定间隔（每 N 小时/天）、每小时指定分钟、每天或每周指定时刻，以及保留天数。沿用页面“编辑配置/保存/取消”，保存不会立即删除。卡片展示最近与下次检查、已清理数量、释放空间估算和跳过/失败详情。缩短保留期会影响已有目录，超过保留期的未提交修改也可能被删除，请提前保存成果。

清理仅作用于数据库目录旁 `worktrees/<仓库标识>/<run_id>` 中已核验归属的运行工作区，不删除基础仓库、安装快照、下载缓存或数据库。活动父子任务及共享目录、锁占用、归属不明、目录链接和迁移中的目录会跳过；权限或文件占用失败会保留原因。停用或删除配置的仓库仍可回收可信旧 clone；无法确认基础仓库的 linked worktree 保留供人工处理。清理后同步更新运行工作区状态及日志。
