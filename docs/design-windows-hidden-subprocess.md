...
- 保留无窗口、标准输入输出和退出码的严格校验，不增加自动重试；本轮只修改测试、CI 诊断及说明文档。
- 同步子探测超时后先终止整棵进程树，再有界收集剩余管道输出。外层诊断输出写入临时文件，使用有界 `wait`，避免杀死父进程后因后代持有管道而持续等待 EOF。
- Windows CI 显示当前用例名称，并在单个用例超过 120 秒时输出 Python 堆栈，便于定位无进展阶段。
