...
- 精确同名的旧组合分支不参与复用。任务开始前如存在，必须先关闭其未合并 PR / MR 并删除本地、远程同名分支；本次组合分支创建后如流程最终失败或阻塞，必须再次清理本次创建的 PR / MR 和分支。依赖阶段返回 `FAILED` 时，只有完成本 Prompt 规定的阶段恢复才可继续文档阶段。
...
- 返回 `BLOCKED`：判定整条流程阻塞并执行“失败与分支清理”，不调用文档子 Agent，不创建 PR / MR。
- 返回 `FAILED`：保存子 Agent 报告的原始失败原因、提交状态、推送状态和本地修改状态，然后执行“依赖失败后的阶段恢复”。恢复成功后仍将依赖阶段状态保留为 `FAILED`，将 `<文档任务起始SHA>` 设为 `<合并后SHA>` 并继续文档阶段；恢复失败时判定整条流程失败并执行“失败与分支清理”。
...
主 Agent不比较依赖子 Agent报告的文件清单与 Git diff，也不重新判断依赖文件范围。`NO_DEPENDENCY_UPDATE` 或 `UPDATED_AND_PUSHED` 的任一状态校验失败、返回未知状态或无法解析结构化结果时，判定任务失败并执行清理，不调用文档子 Agent；不得把协议异常伪装成可恢复的依赖业务失败。

## 依赖失败后的阶段恢复

本节只适用于依赖子 Agent 明确返回 `FAILED`，不适用于 `BLOCKED`、未知状态、目标分支漂移、平台事实异常或依赖阶段状态校验异常。

1. 保存依赖失败的原始原因以及子 Agent 报告的全部本地、提交和推送状态，后续恢复不得覆盖或改写该失败事实。
2. 查询同项目中 head/source branch 精确等于 `<组合自动更新分支>` 的 PR / MR；关闭并核验所有尚未合并的匹配对象。出现已经合并的匹配对象时视为状态不可恢复，立即执行“失败与分支清理”。
3. fetch 已确定的 remote。如果远程存在精确的 `refs/heads/<组合自动更新分支>`，使用完整引用删除并核验消失；不得保留依赖 Agent 在失败前产生或推送的任何提交。
4. 因调用依赖 Agent 前已经确认工作区干净，调用后出现的已跟踪和暂存修改视为本阶段产生。只对精确列出的本阶段跟踪路径恢复到 `<合并后SHA>`；不得使用宽泛 `reset`、`clean` 或删除范围不明确的文件。
5. 对未跟踪文件，只删除能够根据本轮命令和子 Agent 报告确认由依赖阶段创建的精确路径。存在所有权不明且阻止恢复的文件时，阶段恢复失败。
6. 安全切换到 `<合并后SHA>` 的 detached HEAD；删除本地精确同名组合分支并核验消失。该分支在其他 worktree 检出时阶段恢复失败，不得操作其他 worktree。
7. 再次刷新远程目标分支并确认其完整 HEAD 仍等于 `TARGET_HEAD_AT_START`；确认同名未合并 PR / MR、本地分支和远程分支均不存在。
8. 从精确的 `<合并后SHA>` 重新创建并切换 `<组合自动更新分支>`，确认当前 HEAD 等于 `<合并后SHA>`、工作区和暂存区干净、远程同名分支不存在。
9. 全部核验通过后记录“依赖失败恢复 = 成功”，将 `<文档任务起始SHA>` 设为 `<合并后SHA>` 并继续；任一步失败都记录“依赖失败恢复 = 失败”，保留原始依赖失败原因并执行“失败与分支清理”。
...
- 文档影响分析范围是 `<合并前SHA>..<文档任务起始SHA>`；依赖阶段成功更新时同时考虑依赖提交形成的最终项目状态，依赖阶段失败并恢复时只分析原始 PR / MR 的合并净差异；
- 只能修改文档和文档索引；
...
1. 如果文档阶段返回 `NO_DOCUMENT_UPDATE`，并且依赖阶段返回 `NO_DEPENDENCY_UPDATE` 或返回 `FAILED` 后阶段恢复成功，确认 `<最终自动更新SHA>` 等于 `<合并后SHA>`、远程组合分支不存在，然后执行“无更新清理”；不得推送空分支或创建空 PR / MR。依赖阶段曾返回 `FAILED` 时，最终结果仍必须标记为“部分完成”。
...
PR / MR 描述应简洁包含：原始 PR / MR 链接和编号、固定合并后基线 SHA、依赖阶段结果和摘要、依赖失败时的恢复结果、文档阶段结果和摘要、实际自动更新提交 SHA及验证摘要。
...
只要“本次组合分支已创建 = 是”，任何导致流程无法继续的失败或阻塞都必须先保存原始原因，然后按以下顺序清理。依赖子 Agent 返回 `FAILED` 本身在阶段恢复成功时不立即触发本节；恢复后的文档阶段、平台门禁或其他后续步骤再次失败或阻塞时，仍必须执行本节：
...
- 整体结果是正常完成、部分完成、失败或阻塞；依赖阶段曾返回 `FAILED` 且恢复后继续文档阶段时必须标记为“部分完成”。
...
2. 依赖阶段没有返回 `FAILED` 或 `BLOCKED`，所有实际产生的依赖和文档提交均按规定顺序普通推送到组合分支，目标分支在整个流程及合并前未漂移，唯一的 `auto-update` PR / MR 门禁全部通过、已经成功合并到原始目标分支，且本地和远程组合分支均已删除。

依赖阶段返回 `FAILED`、阶段恢复成功且文档阶段完整结束时，必须标记为部分完成：文档存在更新时，其唯一 `auto-update` PR / MR 必须通过门禁并成功合并；文档无需更新时不得创建空分支或空 PR / MR；两种情况都必须完成本地和远程组合分支清理。部分完成不得描述为依赖更新成功，也不得遗漏原始依赖失败原因。

除上述正常完成或部分完成外，其他情况均标记为失败或阻塞。

现在开始执行。首先解析并验证已经合并的原始 PR / MR，通过防循环检查后生成唯一组合分支，校验两个子 Agent 名称，并按“依赖更新 → 必要时恢复依赖失败 → 增量文档更新 → 单一同平台 PR / MR”的顺序执行。
