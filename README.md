...
“已扫描 MR / PR”和“最近变化事件”都使用服务端分页，共用顶部仓库范围，各自保留多状态、排序方向和展示数量筛选，后者还可以按编号筛选。MR / PR 列表可按编号、远端更新时间、最近扫描时间或最新平台事件时间排序，默认按远端更新时间从新到旧；事件列表可按编号或事件时间排序，默认按事件时间从新到旧。统计卡片不受这些列表条件限制。切换仓库、筛选或排序时清空对应批量勾选并回到第一页。每张列表默认显示 10 条，也可选择 20、50、全部或输入自定义正整数。
