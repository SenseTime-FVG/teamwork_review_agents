...
后台启动只有在管理服务健康接口实际由本次新 PID 响应后才会返回成功；仅写入 PID 文件或短暂持有进程锁不代表服务已经成功监听端口。端口被旧服务或其他程序占用、子进程提前退出、健康检查响应来自旧 PID 时，`start` / `restart` 会返回失败并显示后台日志摘要。

如果运行期间 PID 或锁文件被意外移动、删除或损坏，`stop` / `restart` 会通过系统进程表按 `--managed-child` 和配置文件绝对路径精确恢复托管进程身份，并确认旧 PID 真正退出后再继续。系统不会只凭端口号结束进程，避免误杀其他服务。SQLite 数据库及其 `-wal`、`-shm` 文件仍不得在服务运行期间移动或删除。
