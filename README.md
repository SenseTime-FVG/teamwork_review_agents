...
预算通过 `runtime.context_compaction` 配置，完整示例见 `config_example.yaml`。窗口优先读取 Provider / 模型级 `model_context_windows` 覆盖，其次使用 Codex 模型缓存中的窗口；外部或未知模型默认预算为 131072，不保证等于其真实容量。当前采用 UTF-8 序列化字节数加结构余量作保守估算，不是精确 Token 计数，也不是累计账单用量。默认预留 4096 输出 token，自动阈值改为完整上下文窗口的 95%（`trigger_ratio: 0.95`）；若扣除输出预留后的安全输入上限更低，则提前在该上限触发。目标仍降至输入预算的 50%，尽量保留最近 2 个完整回合。已有显式比例配置继续生效，统一以完整窗口为基数。日志记录窗口来源、实际触发阈值和完整/分片摘要模式。`summary_target_bytes` 默认 2048，只是摘要软目标，不是失败上限；超过目标但完整请求能装下且已缩小时照常接受。旧键 `max_summary_tokens` 仍可读取，保存后使用新名称。每次压缩最多请求 16 次，包含最多额外 2 次收短（`max_summary_rewrites`）。压缩调用会消耗模型额度并计入运行用量，`enabled: false` 只关闭历史摘要，不关闭结果文件与资源保护；完整 Codex CLI 模式仍使用 CLI 自己的压缩机制。
