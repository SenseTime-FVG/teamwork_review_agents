...
托管模型循环默认开启历史压缩：系统指令、权限约束、Skill 正文、工具定义、输出 Schema 和原始任务 Prompt 原样保留，仅将较早的完整执行回合归纳为文本交接摘要。摘要以 assistant 历史材料回传，不变成新的系统指令；近期调用与结果成组保留，摘要请求本身不执行工具。超长工具结果只截短送给模型的副本，完整脱敏结果仍保留在执行日志中。文本摘要有损，不能替代原始证据；关键事实不确定时应重新定向读取文件或查询状态，而不是重复执行提交、推送等已完成操作。

预算通过 `runtime.context_compaction` 配置，完整示例见 `config_example.yaml`。窗口优先读取 Provider / 模型级 `model_context_windows` 覆盖，其次使用 Codex 模型缓存中的窗口；外部或未知模型默认预算为 131072，不保证等于其真实容量。当前采用 UTF-8 序列化字节数加结构余量作保守估算，不是精确 Token 计数，也不是累计账单用量。默认预留 4096 输出 token，达到输入预算的 80% 时压缩，目标降至 50%，尽量保留最近 2 个完整回合。`summary_target_bytes` 默认 2048，只是摘要软目标，不是失败上限；超过目标但完整请求能装下且已缩小时照常接受。旧键 `max_summary_tokens` 仍可读取，保存后使用新名称。单个工具送模结果仍限制为 4096 字节；每次压缩最多请求 16 次，包含最多额外 2 次收短（`max_summary_rewrites`）。压缩调用会消耗模型额度并计入运行用量，`enabled: false` 可关闭这一机制；完整 Codex CLI 模式仍使用 CLI 自己的压缩机制。
