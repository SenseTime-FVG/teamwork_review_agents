...
标题旁的仓库选择器统一控制运行概览，默认“全部仓库”。选择具体仓库后，MR / PR、变化事件、Agent 运行统计及下方两个列表一起切换；最近扫描卡片改为该仓库的最近成功扫描时间，没有记录时显示“暂无记录”。选项使用仓库展示名称，停用但仍在配置中的仓库也可查看历史。后台调度器状态和“立即扫描／暂停／恢复”明确标注为全局，不因筛选而改变作用范围。
