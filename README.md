...
“最近变化事件”支持单条和批量手动触发。每次操作都会复制来源事件保存的快照上下文并创建新的独立手动事件，来源事件及其状态保持不变；相同 Head 与配置版本仍可能复用已有 Preflight / CI 结果。MR / PR 详情会展示最新平台事件参考和触发按钮，关联事件保持左键查看详情，并可通过右键或键盘上下文菜单手动触发。
