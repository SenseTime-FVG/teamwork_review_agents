...
压缩算法仍通过 `runtime.context_compaction` 配置。当前采用 UTF-8 序列化字节数加结构余量作保守估算，不是精确 Token 计数，也不是累计账单用量。默认预留 4096 输出 token，自动阈值为完整上下文窗口的 95%（`trigger_ratio: 0.95`）；若扣除输出预留后的安全输入上限更低，则提前在该上限触发。目标仍降至输入预算的 50%，尽量保留最近 2 个完整回合。已有显式比例配置继续生效，统一以完整窗口为基数。日志记录窗口来源、实际触发阈值和完整/分片摘要模式。`summary_target_bytes` 默认 2048，只是摘要软目标，不是失败上限；超过目标但完整请求能装下且已缩小时照常接受。旧键 `max_summary_tokens` 仍可读取，保存后使用新名称。每次压缩最多请求 16 次，包含最多额外 2 次收短（`max_summary_rewrites`）。压缩调用会消耗模型额度并计入运行用量，`enabled: false` 只关闭历史摘要，不关闭结果文件与资源保护；完整 Codex CLI 模式仍使用 CLI 自己的压缩机制。

`tool_output_inline_bytes` 默认 65536（64KiB），只决定工具结果直接返回正文还是完整文件引用，不是丢弃内容的上限；旧 `tool_output_tokens` 已忽略，旧的 4096 值不会沿用。结果超过阈值，或当前模型窗口扣除固定内容及本轮工具结构后容纳不了时，服务将完整脱敏 JSON 存入本轮已授权运行目录的 `tool-results`，返回路径、字节数、SHA-256、退出码等元数据及可选预览。模型通过现有 `execute_command` 按字段/行/字符范围分段补读，不新增工具，不必重新执行原命令。文件仅本次运行期间有效，结束后随临时目录清理；UI 执行日志仍可追溯。底层命令也不再静默丢弃 1MB 后的输出：单次捕获达到 16MiB 资源保护上限时明确终止，结果文件总量超过运行级 256MiB 预算或写入失败时明确失败且不自动重跑，不能冒充已完整保存。
