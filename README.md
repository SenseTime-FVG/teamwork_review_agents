...
“已扫描 MR / PR”和“最近变化事件”都使用服务端分页，共用顶部仓库范围，各自保留多状态和展示数量筛选，后者还可以按编号筛选。统计卡片不受这些列表条件限制。切换仓库时清空批量勾选并回到第一页；MR / PR 按远端更新时间倒序，事件按实际发生时间倒序。每张列表默认显示 10 条，也可选择 20、50、全部或输入自定义正整数。
