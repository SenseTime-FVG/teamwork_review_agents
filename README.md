...
预算通过 `runtime.context_compaction` 配置，完整示例见 `config_example.yaml`。窗口优先读取 Provider / 模型级 `model_context_windows` 覆盖，其次使用 Codex 模型缓存中的窗口；外部或未知模型默认预算为 131072，不保证等于其真实容量。当前采用 UTF-8 序列化字节数加结构余量作保守估算，不是精确 Token 计数，也不是累计账单用量。默认预留 4096 输出 token，达到输入预算的 80% 时压缩，目标降至 50%，尽量保留最近 2 个完整回合；摘要和单个工具送模结果分别限制为 2048、4096 字节，每次压缩最多请求 16 次。压缩调用会消耗模型额度并计入运行用量，`enabled: false` 可关闭这一机制；完整 Codex CLI 模式仍使用 CLI 自己的压缩机制。

切换模型后会按新模型预算重新检查。上游明确报告上下文超限时，压缩后只重试当前请求一次，不重跑已执行工具；固定 Prompt 自身过大、摘要无效或压缩后仍超限时明确停止整轮自动重试，不靠切换模型或清空工作区掩盖问题。压缩过程中遇到额度/临时上游故障仍按现有回退策略处理。摘要完整生成并验证有效后才替换活跃历史，失败或取消不会提交半份摘要；运行消息与快照提供前后估算量、模型、请求轮次、保留回合及摘要，原始执行日志不改写。
