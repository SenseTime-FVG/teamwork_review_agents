...
class ContextCompactionError(RuntimeError):
    ...
    def __init__(self, message: str, *, error_code: str = "context_compaction_failed", usage: dict[str, Any] | None = None) -> None:
...
def summary_payload(model: str, previous: str, fragment: str, limit: int) -> dict[str, Any]:
    """摘要请求独立于正常任务，无工具、无任务输出 Schema。"""

    source = json.dumps({"previous_summary": previous, "history_fragment": fragment}, ensure_ascii=False)
    return {
        "model": model,
        "instructions": SUMMARY_INSTRUCTIONS + f"最终摘要的 UTF-8 字节数不得超过 {limit}。",
...
class ConversationContext:
    ...
    async def ensure_budget(
        ...
        if not self.rounds and not self.summary:
            ...
            return None
        summary_limit = min(settings.max_summary_tokens, max(128, (hard_limit - fixed_cost) // 4))
        target = max(int(hard_limit * settings.target_ratio), fixed_cost + summary_limit + 256)
        if force:
            target = min(target, max(fixed_cost + summary_limit + 256, int(before * 0.6)))
        keep = min(settings.keep_recent_rounds, len(self.rounds))
        while keep and cost(self._history("x" * summary_limit, self.rounds[-keep:])) > target:
            keep -= 1
        ...
        if not prefix and not self.summary:
            return None
        material = json.dumps(_text_material(prefix), ensure_ascii=False)
        draft = self.summary
        offset = 0
        requests = 0
        ...
        while offset < len(material):
            if requests >= settings.max_compaction_requests:
                raise ContextCompactionError("历史过大，压缩请求已达到本次上限；原历史保留，请缩小单次工具输出")
            low, high = 0, len(material) - offset
            while low < high:
                middle = (low + high + 1) // 2
                payload = summary_payload(model, draft, material[offset:offset + middle], summary_limit)
                if estimate_tokens(payload) + 256 <= hard_limit:
                    low = middle
                else:
                    high = middle - 1
            if low == 0:
                raise ContextCompactionError("模型窗口不足以容纳摘要请求；原历史保留")
            payload = summary_payload(model, draft, material[offset:offset + low], summary_limit)
            draft = (await summarize(payload)).strip()
            requests += 1
            if not draft or len(draft.encode("utf-8")) > summary_limit:
                raise ContextCompactionError("压缩模型返回空摘要或摘要超过预算；原历史保留")
            offset += low
        after = cost(self._history(draft, tail))
        if after >= before and before <= hard_limit and not force:
            # 固定长 Prompt 占比高时，短历史未必适合摘要；尚未超限便保留原历史。
            return None
        if after >= before or after > hard_limit:
            raise ContextCompactionError("压缩未有效缩小上下文或仍然超限；原历史保留")
        self.summary = draft
        self.rounds = tail
        self.compaction_count += 1
        return {
            "before_estimated_tokens": before, "after_estimated_tokens": after,
            "context_window": window, "input_budget": hard_limit,
            "estimator": "utf8_bytes_conservative", "summary_requests": requests,
